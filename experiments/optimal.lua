...
function NumberList:decode(decoder)
    local size  = readsize(decoder.reader, sizebits)
    local value = { }
    
    local read_func   = self.read_func
    local fmtstring   = self.fmtstring
    local packsize    = self.packsize
    
    local reader      = decoder.reader
    local raw_read    = reader.raw
        
    for i=1, size, count do
        read_func(value, i, sunpack, fstring, raw_read, reader, packsize)
    end
    return value
...
function optimal.number_list(fmt, count, sizebits)
    ...
    list.packsize  = string.packsize(fstring)
    list.packed    = { }

    --Should be a way to do what I want in Lua...
    local read_values = "local function read_the_values(value, i, sup, fstr, rr, reader, ss) \n\t"
    for i=0, count - 1 do
    ...
    read_values = read_values .. " = sup(fstr, rr(reader, ss)) \nend \nreturn read_the_values"
    self.read_func = load(read_values)()
