...
function format.writer(inner)
	local Writer = { ["inner"] = inner }
	local position   = 0
	local bit_count  = 0
	local bit_buffer = 0
	local inner_write = inner.write
	...
	local function writevarint(writer, number)
	   ...
	   inner_write(inner, spack(bytesPack[count], tunpack(tmpByteStorage)))
	...
	function Writer:int32(number)   
	   inner_write(inner, spack("i4", number))
	   position = position + 4
	end
	...
	function Writer:bool(bool)
	   ...
	   inner_write(inner, spack("B", bool))
	   position = position + 1
	end
