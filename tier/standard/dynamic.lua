...
return function (standard)
    do 
        ...
        standard.descriptive = custom.dynamic(dynamic_handler, standard.type)
        standard.generator:register_mapping(standard.descriptive)
    end
    
    do 
    	assert(standard.type, "Meta type mapping standard.type must be initialized before this module is loaded!")
    	local Preserving = { meta = meta.dynamic }
        local type       = standard.type
        function Preserving:encode(encoder, value, ...)
        	local mapping = value.mapping
        	local data    = value.data
        	type:encode(encoder, mapping)
        	mapping:encode(encoder, data, ...)
        end 
        
        function Preserving:decode(decoder, ...)
        	local mapping = type:decode(decoder, value)
        	local data    = mapping:decode(decoder, ...)
        	return { ["mapping"] = mapping, ["data"] = data }
        end
   
        standard.preserving = Preserving
        print(standard.preserving)
    end
    ...
    do 
    	...
    	local dynamic = custom.dynamic(lua2tag, standard.type)
